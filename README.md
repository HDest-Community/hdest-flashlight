# HDest Flashlight
_Originally "DarkDoomZ (Ace Bootleg)", made by Accensus, now maintained by the communtiy._  
_**NOTE: No longer includes DDZ, feel free to use whatever relighting mod you wish.**_  

Small mod that adds a Flashlight item that requires Cell Batteries to operate (press Use + Use Item to pull up the flashlight).  The flashlight will start flickering when your battery is low, so make sure to keep them stocked up!

## Notes
- Loadout Code: `ddf`
- The flashlight can only spawn in backpacks.
- To bring the flashlight up, press Use + Use Item.

## Credits

### Code
- Original Credit for any remaining DDZ code (if any) belongs to its contributors:
  - Kinsie: Fancy Doom
  - Gutawer: Code template for per-sector iteration
  - FishyClockwork: Lighting adjustment code
  - phantombeta, Marisa Kirisame: Sector light effect removal suggestion and code
  - Caligari87: Original DarkDoomZ re-implementation
- For the additional code, refactoring, modularity:
  - Accensus: Iterator for extraneous actor removal, addon support
  - Undead_Zeratul: Post-community enhancements, cleanup, etc.

### Sounds
- mshahen: "Flashlight On/Off Clicks" (freesound.org, 271109, modified) CC BY 3.0
- Monolith Productions: Lighter Open/Light Noises (Blood)
- CutmanMike: Lighter Close (Ghoul's Forest)

### Testing
- Fort of Hard Knox, a1337spy, Orange Bomb: Multiplayer flashlight testing in Hideous Destructor

### Graphics
- Accensus: Nice logo for DarkDoomZ
- Pillowblaster: Flashlight Sprite
- XLightningStormL: Flood Lamp Sprite
- elend: Worklight Sprites (Based on Sketchfab models by GabeTheSerrano)
- CutmanMike: Lighter Pickup Sprite (Ghoul's Forest)
- Monolith Productions, Joblez: Lighter HUD Sprites